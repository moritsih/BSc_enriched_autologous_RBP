...
def calc_coverages_autol_len(len_normalize=False, consider_overlap=False, background_longer_than_autol_only=False):
        ...
        for subseq in matches[exp].keys( ):
            ...
            for motif in matches[exp][subseq].keys( ):
                ...
                for seq in matches[exp][subseq][motif].values(): #all the matches a motif has with a sequence
                    ...
                    autol_len = len(MANE_transcriptome[motif][subseq]) # get the length of the autologous sequence
                    seq_len = seq[0][4]
